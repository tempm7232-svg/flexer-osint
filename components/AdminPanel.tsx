...
const AdminPanel: React.FC<AdminPanelProps> = ({ profile, onLogout }) => {
  ...
  const handleDeleteUser = async (user: UserProfile) => {
    ...
    if (confirm(`Expunge operative ${user.email}?`)) {
      await deleteDoc(doc(db, 'users', user.uid));
    }
  };

  const handleDeleteTool = async (toolId: string, toolName: string) => {
    if (window.confirm(`Permanently delete the module "${toolName}"?`)) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      <header className="bg-[#111] border-b border-gray-800 p-4 md:p-6 flex flex-col md:flex-row justify-between items-center sticky top-0 z-50 gap-4">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex gap-1">
            <button onClick={() => setIsPasswordModalOpen(true)} className="p-2 text-gray-500 hover:text-blue-500"><i className="fas fa-key"></i></button>
            <button onClick={onLogout} className="p-2 text-gray-500 hover:text-red-500"><i className="fas fa-power-off"></i></button>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-6">
                ...
                <tbody className="divide-y divide-gray-800">
                  {users.map(u => (
                    <tr key={u.uid} className="hover:bg-gray-800/10 group transition">
                      <td className="p-6 text-sm text-white font-bold">{u.email}</td>
                ...
                <div key={u.uid} className="bg-[#111] border border-gray-800 rounded-xl p-4 shadow-lg space-y-3">
                  ...
                  {!u.isOwner && (
                    <div className="flex gap-2 pt-2 border-t border-gray-800/50">
                      ...
                      <button onClick={() => handleDeleteUser(u)} className="px-3 bg-gray-800 text-gray-400 rounded-lg"><i className="fas fa-trash-alt"></i></button>
                    </div>
                  )}
                </div>
              ))}
        ...
        {activeTab === 'tools' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center"><h2 className="text-xl font-bold text-white uppercase tracking-tighter">Modules</h2><button onClick={() => { setEditingTool({}); setIsToolModalOpen(true); }} className="bg-blue-600 px-6 py-2 rounded-xl font-bold text-xs"><i className="fas fa-plus mr-2"></i>ADD</button></div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {tools.map(tool => (
                <div key={tool.id} className="bg-[#111] border border-gray-800 rounded-2xl p-6 shadow-xl flex flex-col hover:border-blue-500/30 transition h-full">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="flex gap-2 text-gray-600">
                      ...
                      <button onClick={() => handleDeleteTool(tool.id, tool.name)} className="hover:text-red-500"><i className="fas fa-trash"></i></button>
                    </div>
                  </div>
            ...
            {securityRequests.length === 0 ? <p className="text-gray-600 text-center py-20 uppercase text-xs font-bold">No Pending Requests</p> : (
              <div className="grid gap-4">
                {securityRequests.map(u => (
                  <div key={u.uid} className="bg-[#111] border border-blue-500/20 rounded-2xl p-6 flex items-center justify-between">
                    <div><div className="font-bold text-white">{u.email}</div><div className="text-[10px] text-gray-500">{u.pendingSessionMetadata?.deviceName}</div></div>
                    <button onClick={() => handleAuthorizeSession(u)} className="bg-blue-600 px-6 py-2 rounded-xl text-xs font-bold text-white">GRANT ACCESS</button>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="space-y-6">
            ...
            <div className="grid gap-4">
              {profile.authorizedSessions?.map(s => (
                <div key={s.sid} className={`bg-[#111] border ${s.sid === currentSid ? 'border-blue-500/50' : 'border-gray-800'} rounded-2xl p-6 flex justify-between items-center`}>
                  <div><div className="font-bold text-white">{s.deviceName} {s.sid === currentSid && '(THIS)'}</div><div className="text-[10px] text-gray-500">{new Date(s.timestamp).toLocaleString()}</div></div>
                  {s.sid !== currentSid && <button onClick={() => handleRevokeDevice(s.sid)} className="text-red-500 text-xs font-bold uppercase tracking-widest border border-red-900/20 px-4 py-2 rounded-lg">Revoke</button>}
                </div>
              ))}
            </div>
          </div>
      ...
      {isToolModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
          <div className="bg-[#111] border border-gray-800 w-full max-w-2xl rounded-3xl p-6 md:p-8 flex flex-col max-h-[90vh]">
            <h3 className="text-xl font-bold text-white mb-6 uppercase">Config Module</h3>
            <div className="space-y-4 overflow-y-auto pr-2">
              <input type="text" className="w-full bg-[#0a0a0a] border border-gray-800 rounded-xl p-3 text-white focus:border-blue-500 outline-none" placeholder="Name" value={editingTool?.name || ''} onChange={e => setEditingTool({...editingTool, name: e.target.value})}/>
              <input type="text" className="w-full bg-[#0a0a0a] border border-gray-800 rounded-xl p-3 text-white focus:border-blue-500 outline-none" placeholder="API Endpoint" value={editingTool?.apiUrl || ''} onChange={e => setEditingTool({...editingTool, apiUrl: e.target.value})}/>
            ...
            <div className="flex gap-4 mt-6">
              <button onClick={handleSaveTool} className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold text-xs">SAVE</button>
              <button onClick={() => setIsToolModalOpen(false)} className="flex-1 bg-gray-800 text-gray-400 py-3 rounded-xl font-bold text-xs">CLOSE</button>
            </div>
          </div>
        </div>
      )}
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md">
          <div className="bg-[#111] border border-gray-800 w-full max-w-md rounded-2xl p-8">
            <h3 className="text-xl font-bold text-white mb-6 uppercase">Change Key</h3>
            <form onSubmit={handleChangePassword} className="space-y-4">
              <input type="password" required className="w-full bg-[#0a0a0a] border border-gray-800 rounded-xl p-3 text-white focus:border-blue-500 outline-none" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="New Password"/>
              <div className="flex gap-4 pt-4">
                <button type="submit" disabled={passwordLoading} className="flex-1 bg-blue-600 text-white font-bold py-3 rounded-xl transition disabled:opacity-50 text-xs">UPDATE</button>
                <button type="button" onClick={() => setIsPasswordModalOpen(false)} className="flex-1 bg-gray-800 text-gray-500 font-bold py-3 rounded-xl transition text-xs">CANCEL</button>
              </div>
            </form>
          </div>
        </div>
